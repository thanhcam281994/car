...
function tm_video($post_id,$file, $url, $code){
	$shortcode = '';
	if(!empty($file)){
		...
		if(has_post_thumbnail( $post_id )){
			$atts['poster']= get_featured_img($post_id);
		}
		$shortcode = '[videojs '.$atts['type'].'="'.$atts['file'].'" poster="'.$atts['poster'].'" height="460" width="100%"]';
	}elseif(!empty($code)) {
		$shortcode  = $code;
	}elseif(!empty($url)) {
		if(has_post_thumbnail( $post_id )){
		...
		if(strpos($url, 'vimeo.com')) {
			$parseVideos = parseVideos($url);
			$shortcode = '<iframe src="'.$parseVideos[0]['url'].'?badge=0" width="100%" height="460" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe> ';
		}
		if(strpos($url, 'youtube.com')) {
			$shortcode = '[videojs youtube="'.$url.'" poster="'.$atts['poster'].'" height="460" width="100%"]';
		}
	}
